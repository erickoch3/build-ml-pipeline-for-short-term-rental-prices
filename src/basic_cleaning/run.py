...
def go(args):
    ...
    # Example: fill missing reviews with a placeholder date and missing names with "unknown"
    df['last_review'] = pd.to_datetime(df['last_review'], errors='coerce')
    df['last_review'].fillna(pd.Timestamp('1900-01-01'), inplace=True)
    df['name'].fillna('unknown', inplace=True)
    df.dropna(inplace=True)  # Alternatively, you can use more sophisticated imputation
    ...
    # Filter minimum night outliers
    idx = df['minimum_nights'].between(0, 10)
    df = df[idx].copy()
